...
export default function ReliabilityTab() {
  ...
  const attendance = useMemo(
    () => [...players].filter(p => p.showed > 0).sort((a, b) => b.showed - a.showed).slice(0, TOP_N),
    [players]
  );

  const guestBoard = useMemo(
    () => players.filter(p => p.guestsBrought > 0).sort((a, b) => b.guestsBrought - a.guestsBrought).slice(0, TOP_N),
    [players]
  );
  ...
  const sortRows = useMemo(() => {
    ...
    return (list: ReliabilityPlayer[]) => [...list].sort((a, b) => cmp[sortKey](a, b) * dir).slice(0, TOP_N);
  }, [sortKey, sortDir]);

  const qualifiedRows = useMemo(() => sortRows(players.filter(p => p.committed >= MIN_GAMES)), [players, sortRows]);
  const unqualifiedRows = useMemo(() => sortRows(players.filter(p => p.committed < MIN_GAMES && p.responded > 0)), [players, sortRows]);
  ...
  const RelTable = ({ rows }: { rows: ReliabilityPlayer[] }) => (
    rows.length === 0 ? (
      <p className="text-text-tertiary text-center py-4 text-sm">No players here.</p>
    ) : (
  ...
  return (
    <div className="space-y-3">
      ...
      <Section
        title="Attendance"
        subtitle={`Top ${TOP_N} by turnout, across ${totalTracked} game${totalTracked === 1 ? '' : 's'}. Admin-only.`}
      ...
      {guestBoard.length > 0 && (
        <Section
          ...
          subtitle={`Top ${TOP_N} inviters — ${totalGuests} guest head${totalGuests === 1 ? '' : 's'} flagged.`}
          open={open.guests}
      ...
      {!hasRsvpData ? (
        <div className="p-3 rounded-lg border border-gold/40 bg-gold/5 text-[12px] text-text-secondary">
          No in-app RSVP data yet. Reliability fills once players RSVP via the app link, or an admin backfills a game.
        </div>
      ) : (
        <>
          <Section
            title="Reliability — Qualified"
            subtitle={`${MIN_GAMES}+ "In" votes · top ${TOP_N}. Rely% = showed of games said In. Tap a column to sort (again to flip).`}
            open={open.qualified}
            onToggle={() => toggle('qualified')}
          >
            <RelTable rows={qualifiedRows} />
          </Section>

          <Section
            title="Reliability — Unqualified"
            subtitle={`Under ${MIN_GAMES} "In" votes · top ${TOP_N}. Thin sample — read with caution.`}
            open={open.unqualified}
            onToggle={() => toggle('unqualified')}
          >
            <RelTable rows={unqualifiedRows} />
          </Section>
        </>
      )}
    </div>

      ...
      <table className="w-full border-collapse">
        <thead>
          <tr className="border-b-2 border-gold">
            <th className="text-left py-3 px-4 font-semibold text-text-secondary">Rank</th>
            <th className="text-left py-3 px-4 font-semibold text-text-secondary">Partners</th>
            <th className="text-left py-3 px-4 font-semibold text-text-secondary">Goal Contributions</th>
          </tr>
        </thead>
        <tbody>
          {partnerStats.length === 0 ? (
            ...
            partnerStats.map((pair, index) => (
              <tr key={`${pair.player1.id}-${pair.player2.id}`} className="border-b border-border hover:bg-surface-hover">
                <td className="py-3 px-4 text-text-secondary font-medium">{index + 1}</td>
                <td className="py-3 px-4">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="font-medium text-text-primary">{pair.player1.name}</span>
                    ...
                    <span className="font-medium text-text-primary">{pair.player2.name}</span>
                  </div>
                </td>
                <td className="py-3 px-4 text-text-secondary">{pair.contributions}</td>
              </tr>
            ))
          )}
        </tbody>
